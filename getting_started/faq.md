...
Detis started as an internal tool for a specific game. Following interest from other developers, it is now available as a public release. 

You get one folder, one executable, and your game inside it. No launcher or account wall to start building.
...
Windows and Linux on **64-bit PC**. **macOS**, mobile, and console are out of scope today.
...
Change assets on disk, then use **Debug → Reload Assets** (`Shift+R`) in the editor to pick up shaders, textures, materials, sounds, animations, scripts, and related data. Worlds also load on demand, which keeps load times short and makes linking smaller worlds into a larger game practical.
...
Not as a built-in streaming open world. Detis is built for authored **worlds**, not infinite terrain streaming.

You can still build large games by linking multiple smaller **`.world`** files together. Each world stays loadable and editable on its own.

## Does it support multiplayer?

No. Detis targets single-player PC games.

Networking is not planned for the near term.

## Does it include global illumination (GI)?

Not built in today. Lighting is built around artist-controlled bounce. Sun, sky fill, local lights, exposure, and materials you tune in the editor.

A dedicated GI solution is planned for a future update. The current renderer is already capable for the look Detis is going for without waiting on that system.

## Does it support custom shaders?

Today you can write custom post-processing effects as GLSL fragment shaders and drive them from Lua. Custom surface shader materials are planned for a future update. The **default material types** shipped with the engine are already more than capable for a full game.
...
You get a binary release with a deep Lua scripting API, which covers what most developers need to make a full game. Native C++ extension and source access may be added later if there is enough demand.
